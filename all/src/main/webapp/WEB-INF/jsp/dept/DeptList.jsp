...
function fn_egov_select(id) {
	document.getElementById("listForm").id.value = id;
   	document.getElementById("listForm").action = "<c:url value='/dept/updateDeptView.do'/>";
   	document.getElementById("listForm").submit();
}

/* 글 등록 화면 function */
function fn_egov_addView() {
   	document.getElementById("listForm").action = "<c:url value='/dept/addDeptView.do'/>";
   	document.getElementById("listForm").submit();		
}
...
function fn_egov_link_page(pageNo){
	...
	document.getElementById("listForm").action = "<c:url value='/dept/DeptList.do'/>";
   	document.getElementById("listForm").submit();
}
	...
	<div id="table">
		<table width="100%" border="0" cellpadding="0" cellspacing="0">
			...
			<tr>
						...
						<td align="center" class="listtd"><c:out value="${result.dname}"/>&nbsp;</td>
						<td align="center" class="listtd"><c:out value="${result.location}"/>&nbsp;</td>
						<td align="center" class="listtd"><c:out value="${result.managerId}"/>&nbsp;</td>
				    			</tr>
			</c:forEach>
		</table>
	</div>
