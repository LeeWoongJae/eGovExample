...
<%
 /**
  * @Class Name : DeptRegister.jsp
  * @Description : Dept Register 화면
...
<c:set var="registerFlag" value="${empty deptVO.id ? '등록' : '수정'}"/>
...
function fn_egov_selectList() {
   	document.getElementById("detailForm").action = "<c:url value='/dept/DeptList.do'/>";
   	document.getElementById("detailForm").submit();		
}

/* 글 삭제 function */
function fn_egov_delete() {
   	document.getElementById("detailForm").action = "<c:url value='/dept/deleteDept.do'/>";
   	document.getElementById("detailForm").submit();		
}
...
function fn_egov_save() {	
	...
	/* TODO Validation기능 보완 */
  	...
  	frm.action = "<c:url value="${registerFlag == '등록' ? '/dept/addDept.do' : '/dept/updateDept.do'}"/>";
    frm.submit();
...
<form:form modelAttribute="deptVO" name="detailForm" id="detailForm" >
<div id="content_pop">
	<!-- 타이틀 -->
	<div id="title">
		<ul>
	...
	<table width="100%" border="1" cellpadding="0" cellspacing="0" >
	   ...
	   <tr>
		...
		<tr>
			<th>DNAME</th>
			<td>
				<form:input path="dname" cssClass="txt"/>
		...
		<tr>
			<th>MANAGER_ID</th>
			<td>
				<form:input path="managerId" cssClass="txt"/>
